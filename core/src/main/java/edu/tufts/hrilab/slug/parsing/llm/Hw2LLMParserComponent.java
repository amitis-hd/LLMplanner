 ...
 */
 ...
 import edu.tufts.hrilab.slug.parsing.llm.ParserRequestBody;
 ...
 public class Hw2LLMParserComponent extends DiarcComponent implements NLUInterface {
 
   public String endpoint = "http://vm-llama.eecs.tufts.edu:8080";
  ...
  public AlternateResponse parseIt(String input) {
    ...
    String prompt = "Parse this input text into a list of \"Referents\" and \"Descriptors\" in addition to an \"Intention\" and return a only the result as a json (a string wraed in {}): " + input 
    + "Here is an example Json for the input go East: {\"referents\":[{\"text\":\"east\",\"type\":\"location\", \"role\":\"central\",\"variable_name\":\"VAR0\",\"cognitive_status\":\"DEFINITE\"}], \"intention\": {\"intent\":\"instruct\",\"proposition\":{\"text\":\"goEast\",\"type\":\"action\",\"arguments\":[]}}, \"descriptors\":[{\"text\":\"east\",\"arguments\":[]}]}";
    
    
    ParserResponse pr = new ParserResponse();

    try {
      Completion answer = TRADE.getAvailableService(new TRADEServiceConstraints().name("chatCompletion").argTypes(String.class)).call(Completion.class, prompt);
      Gson gson = new Gson();
      JsonObject jsonObject = gson.fromJson(answer.getText(), JsonObject.class);
      log.info("this is the json" + jsonObject);
    
      pr = gson.fromJson(jsonObject, ParserResponse.class);
      log.info("LLM call successful, received answer: " + answer.getText());
    } catch (TRADEException e) {
      e.printStackTrace();
      log.error("TRADE service call failed.", e);
    }
